...
df = pd.read_csv('data/apt_data_geo_knn.csv', low_memory=False)
print(f"  원본 세대수: {len(df):,}")

# 결측치 제거
df = df.dropna(subset=['median_price', 'land_price', 'lat', 'lon',
                        'dist_cbd', 'dist_subway', 'dist_park', 'slope',
...
# MinMax 스케일링 (slope, age, dist 변수)
scaler = MinMaxScaler()
scale_cols = ['age', 'dist_cbd', 'dist_subway', 'dist_park', 'slope']
...
# 독립변수 정의
INDEP_VARS = ['log_land_price', 'age', 'log_priv_area', 'floor_clean',
              'dist_cbd', 'dist_subway', 'dist_park', 'slope']
...
for apt_type, type_name in type_map.items():
    ...
    for i, var in enumerate(INDEP_VARS):
        coef_comparison.append({
            ...
            'slm_coef': slm_m.betas[i+1][0],
            'ols_pval': ols_m.pvalues[var] if var in ols_m.params else np.nan,
        })
...
out_cols = ['rid', 'apt_code', 'bj_dong_name', 'road_name', 'apt_name',
            ...
            'dist_cbd', 'dist_subway', 'dist_park', 'slope', 'age',
            'land_price', 'ols_pred', 'slm_pred',
