# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 15:03:12 2026

@author: golds
"""

"""
Step 3: 공간 지연 모형(SLM) 기반 적정 공시가격 추정
=====================================================
방법론:
  - 전체 데이터에 SLM(공간 지연 모형) 단일 적용
  - Hedonic OLS와 성능 비교 (선행연구 대비 개선 검증)
  - SLM 추정 실거래가 × 현실화율 → 적정 공시가격 역산
...
rows, cols = zip(*[~transform * coord for coord in coords])

rows = np.array(rows).astype(int)
cols = np.array(cols).astype(int)

# slope 값 추출
df["slope"] = slope_deg[rows, cols]

# 이상값 제거
df = df[(df["slope"] >= 0) & (df["slope"] <= 60)].reset_index(drop=True)
...
LOG_FEATURES = [
    ...
    "floor_clean", "dist_cbd", "dist_subway", "dist_park","slope"
]

LABELS = [
    "log(공시지가)", "건축경과연수", "log(전용면적)",
    "층수", "업무지구거리", "지하철역거리", "근린공원거리","경사도"
]
...
scaler = MinMaxScaler()
df[LOG_FEATURES] = scaler.fit_transform(df[LOG_FEATURES])
...
# 5. 주 모형: SLM (공간 지연)
# ─────────────────────────────────────────
print("\n▶ [주 모형] SLM(공간 지연 모형) 추정 중... (수 분 소요)")
"""
OLS 진단 결과:
  Lagrange Multiplier (lag) p=0.000 → SLR 채택
...
slm = GM_Lag(
    ...
    name_y="log(실거래가)",
    name_x=LABELS,
    name_ds="관악구 SLR(공간지연모형)",
    robust="white",
)
print(slm.summary)

df["slm_pred_price"] = np.exp(slm.predy.flatten())
slm_r2   = slm.pr2
slm_rmse = np.sqrt(np.mean((y.flatten() - slm.predy.flatten())**2))
slm_mae  = np.mean(np.abs(df[TARGET].values - df["slm_pred_price"].values))
print(f"\n  SLR pseudo-R²: {slm_r2:.4f}  RMSE(log): {slm_rmse:.4f}  MAE(만원): {slm_mae:,.0f}")

# ─────────────────────────────────────────
# 6. 모형 비교 요약
# ─────────────────────────────────────────
print("\n" + "="*60)
print(" 모형 비교 — Hedonic OLS vs SLM")
print("="*60)
print(f"  {'지표':<20} {'Hedonic OLS':>14} {'SLM':>14}")
print(f"  {'-'*50}")
print(f"  {'R²':<20} {ols_r2:>14.4f} {slm_r2:>14.4f}")
print(f"  {'RMSE(log)':<20} {ols_rmse:>14.4f} {slm_rmse:>14.4f}")
print(f"  {'MAE(만원)':<20} {ols_mae:>14,.0f} {slm_mae:>14,.0f}")
improvement = (ols_rmse - slm_rmse) / ols_rmse * 100
print(f"\n  RMSE 개선율: {improvement:.1f}% (OLS 대비 SLM)")
print("="*60)
...
df["tier_ratio"] = df["price_tier"].map(tier_ratio)
df["tier_ratio"] = pd.to_numeric(df["tier_ratio"], errors="coerce")

df["final_ratio"] = np.where(
    group_size >= 5,
    df["group_ratio"],
    df["tier_ratio"]
)
...
# 결측 처리
df["final_ratio"] = pd.to_numeric(df["final_ratio"], errors="coerce")
df["final_ratio"] = df["final_ratio"].fillna(df["assess_ratio"].median())
...
# ── 4단계: 적정 공시가격 산출
df["fair_price"] = df["slm_pred_price"] * df["final_ratio"] * 10000
...
fig.suptitle("Hedonic OLS vs SLM — 실제 vs 예측 실거래가", fontsize=13)
for ax, col, title, color in zip(
    axes,
    ["ols_pred_price", "slm_pred_price"],
    [f"Hedonic OLS  (R²={ols_r2:.3f})", f"SLM  (R²={slm_r2:.3f})"],
    ["#378ADD", "#E24B4A"]
):
...
plt.close()
print("  data/figures/ols_vs_slm.png 저장 완료")
...
ax.set_ylabel("세대 수")
ax.set_title("공시가격 형평성 비율 분포 (SLM 기준)")
ax.legend()
plt.tight_layout()
...
ax.set_title("관악구 공시가격 형평성 공간 분포 (SLM 기준)", fontsize=11)
ax.set_xlabel("경도")
ax.set_ylabel("위도")
ax.set_aspect("equal")
plt.tight_layout()
...
result_df = df[[
    ...
    "slm_pred_price",    # SLM 예측 실거래가 (만원, 주 모형)
...
with open("data/model_summary.txt", "w", encoding="utf-8") as f:
    ...
    f.write(f"SLM(공간 지연 모형)\n")
    f.write(f"  pseudo-R² : {slm_r2:.4f}\n")
    f.write(f"  RMSE(log) : {slm_rmse:.4f}\n")
    f.write(f"  MAE(만원) : {slm_mae:,.0f}\n\n")
